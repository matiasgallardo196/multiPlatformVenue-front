...
import { Plus, MoreHorizontal, Eye, Edit, Trash2, Loader2, User, Search } from "lucide-react"
...
export default function PersonsPage() {
  ...
  const filteredPersons = useMemo(() => {
    ...
    return persons.filter((person) => {
      const fullName = [person.name, person.lastName, person.nickname].filter(Boolean).join(" ").toLowerCase()
    ...
    return (
      <DashboardLayout>
        ...
        <div className="text-center py-8">
          <p className="text-destructive">Error loading persons: {error.message}</p>
        </div>
      </DashboardLayout>
    )
  }
  ...
  return (
    <DashboardLayout>
      <PageHeader title="Persons" description="Manage individual person records and their information">
        <Button>
          ...
          <div className="text-center py-8">
            {persons?.length === 0 ? (
              <>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Get started by creating a new person.</p>
                <div className="mt-6">
                  <Button>
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Person
              ...
              <p className="text-muted-foreground">No persons match your search criteria.</p>
            )}
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Showing {filteredPersons.length} of {persons?.length || 0} persons
              </p>
            </div>
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold">{getPersonName(person)}</h3>
                          {person.nickname && person.nickname !== getPersonName(person) && (
                  ...
                  <CardContent className="space-y-3">
                    {person.incidents && person.incidents.length > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline">{person.incidents.length}</Badge>
                      </div>
                    )}
                    ...
                    {person.imagenProfileUrl && person.imagenProfileUrl.length > 1 && (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-1">
                          {person.imagenProfileUrl.slice(1, 4).map((url, index) => (
                            <Avatar key={index} className="h-6 w-6">
                              <AvatarImage src={url || "/placeholder.svg"} alt={`Photo ${index + 2}`} />
